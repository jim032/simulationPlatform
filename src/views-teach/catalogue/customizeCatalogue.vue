          ...
          <div class="scat-table">
            <ul>
              <li class="sitem" v-for="(item, index) in cataList" :key="index">
                <p
                  :class="{
                    arrow: item.category_list && item.category_list.length
                  }"
                  @click="linkRouter(item, item.course_name)"
                >
                  <span class="snum">{{ (index + 1) | catIndex }}</span
                  >{{ item.course_name }}
                </p>
                <div class="listbox" v-if="item.ishow">
                  <span
                    class="ii-item"
                    v-for="(iitem, index) in item.category_list"
                    @click="linkRouter(iitem, item.course_name)"
                    >{{ iitem.category_name_en }}</span
                  >
                </div>
              </li>
            </ul>
    ...
    linkRouter(obj, pname) {
      ...
      if (!obj.category_list) {
        ...
        if (text == "e8d78d7e-f984-11ea-adc1-0242ac120002") {
          that.$router.push({
            name: "thesaurus",
            params: { id: text, name: name, pname: pname }
          });
        }
        //初始区块链
        ...
        if (text == "12d4a962-f98b-11ea-adc1-0242ac120002") {
          that.$router.push({
            name: "51attack",
            params: { id: text, name: name.replace(/%/g, "") }
          });
        }
