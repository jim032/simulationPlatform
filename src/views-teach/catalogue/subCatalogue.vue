        ...
        <div class="scataList">
          <ul>
            <li class="sitem" v-for="(item, index) in cataList" :key="index">
              <p
                class="cursorPoint"
                :class="{ arrow: item.children && item.children.length }"
                @click="linkRouter(item, index)"
              >
                <span class="snum">0{{ index + 1 }}</span
                >{{ item.name }}
              </p>
              <div class="listbox" v-if="item.ishow">
                <span
                  class="ii-item cursorPoint"
                  v-for="(iitem, index) in item.children"
                  @click="linkRouter(iitem, index)"
                  >{{ iitem.name }}</span
                >
              </div>
            </li>
    ...
    linkRouter(obj, index) {
      ...
      if (that.courseId == "890affa2-f984-11ea-adc1-0242ac120002") {
        ...
        if (text == "e8d78d7e-f984-11ea-adc1-0242ac120002") {
          that.$router.push({
            name: "thesaurus",
            params: { id: obj.id, name: obj.name, pname: that.courseName }
          });
        }
        //初始区块链
