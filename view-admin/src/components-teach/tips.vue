...
'提示栏进行下一步操作。':'获胜节点已完成区块打包并顺利广播。为了犒劳该节点的辛苦工作，系统将给予一定奖励！至此，节点模拟运行已结束，用户可点击右上角的重置进行重新操作，或返回主界面选择其他篇章。'	  
			  ...
			  if(that.pageName=='2'){
			    switch(that.step){
				   case 1:
				   	that.confirmInfo = '当前节点网络中已有四名用户，请按照右侧步骤进行下一步操作。'
				   	break;
				   case 2:
				   	that.confirmInfo = '总结上一部分的操作内容(币种设置)，告知币种已设置完成，按照右侧步骤提示进行下一步操作。'
				   	break;
				   case 3:
				   	that.confirmInfo = '当前智能合约已部署完毕，请按照右侧步骤提示进行下一步转账操作。'
				   	break;
			   }				
				   return;
				}
				   ...
				   switch(that.step){
					   case 0:
					    that.tipTiltle = '哈希算法的基本概念及特点'
					   	that.confirmInfo = '哈希算法是区块链中保证交易信息不被篡改的单向密码机制。哈希算法接收一段明文后，以一种不可逆的方式将其转化成一段长度较短、'+
					   	'位数固定的散列数据。它有两个特点：一是加密过程不可逆，意味着我们无法通过输出的散列数据倒推原本的明文是什么；'+
					   	'二是输入的明文与输出的散列数据一一对应，任何一个输入信息的变化，都必将导致最终输出的散列数据的变化。'
					   	break;
					   case 1:
					    that.tipTiltle = '请注意';
